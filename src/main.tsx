...
type DocName = "proposal.md" | "design.md" | "tasks.md";

type ChangeStatus = "draft" | "ready" | "blocked";
...
function App() {
  ...
  function setLocale(nextLocale: Locale) {
    ...
    localStorage.setItem("openspec-companion-locale", nextLocale);
      ...
      <section className="workspace">
        <aside className="sidebar">
          <div className="sidebar-header">
            ...
            <button className="icon-button compact" title={t("generateDraft")} onClick={createDraftPreview} disabled={busy}>
              <Plus size={17} />
            </button>
          </div>
          ...
          <label className="search">
            <Search size={16} />
            <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={t("searchChanges")} />
          </label>
            ...
            {filteredChanges.map((change) => (
              ...
              >
                ...
                <span>
                  ...
                  <small>{change.isPreview ? t("previewPrefix") : ""}{change.summary}</small>
                </span>
            ...
            <>
              <div className="editor-header">
                <div>
                  ...
                  <h2>{displayedChange.id}</h2>
                </div>
                <div className="doc-tabs">
                  {docNames.map((name) => (
                    <button key={name} className={activeDoc === name ? "selected" : ""} onClick={() => setActiveDoc(name)}>
                      <FileText size={15} />
                      {name}
                    </button>
                  ))}
              ...
              <textarea
                ...
                value={displayedChange.docs[activeDoc]}
                onChange={(event) => updateDoc(event.target.value)}
                spellCheck={false}
                readOnly={Boolean(patchPreview)}
              />
            </>
          ) : (
